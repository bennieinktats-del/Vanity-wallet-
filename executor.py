...
def cmd_transact(parts):
    ...
    try:
        ...
        if not TRONGRID_API_KEY:
            raise ValueError(
                "TRONGRID_API_KEY is missing. Add the GitHub Actions secret before using /transact."
            )

        # Use the configured TronGrid API key for every TronPy HTTP request.
        # Without this, TronPy can reach /wallet/getnodeinfo without the
        # required authentication header and TronGrid returns HTTP 401.
        tron = Tron(HTTPProvider(api_key=TRONGRID_API_KEY))
        actual_c = c_priv.public_key.to_base58check_address()

        # Validate TRON Base58Check addresses locally.  Do not use
        # tron.address.to_hex() here: in tronpy 0.6.2 that helper is not a
        # reliable public validation interface and can falsely reject a valid
        # Base58Check address.
        def validate_tron_address(address, name):
        ...
        for name, address in (
            ...
            validate_tron_address(address, name)
...
def cmd_execute():
    ...
    try:
        ...
        if not TRONGRID_API_KEY:
            raise ValueError("TRONGRID_API_KEY is missing.")
        tron = Tron(HTTPProvider(api_key=TRONGRID_API_KEY))
