...
MANUAL_WALLET_C_PRIVATE_KEY = os.getenv("MANUAL_WALLET_C_PRIVATE_KEY", "")
...
def cmd_manual(parts):
    """Register an existing A -> B pair and manually supplied Wallet C.

    Wallet C's private key is read only from the GitHub Actions environment;
    it is never accepted through Telegram.
    """
    log.info("Command received: manual")

    if len(parts) != 4:
        send_telegram(
            "❌ Usage: <code>manual [wallet_a] [wallet_b] [wallet_c]</code>\n\n"
            "This immediately starts monitoring Wallet A → Wallet B.\n"
            "Wallet C must match the address derived from the "
            "<code>MANUAL_WALLET_C_PRIVATE_KEY</code> GitHub Secret."
        )
        return

    wallet_a, wallet_b, wallet_c = [x.strip() for x in parts[1:4]]

    try:
        a_hex = tron_base58_to_hex(wallet_a)
        b_hex = tron_base58_to_hex(wallet_b)
        c_hex = tron_base58_to_hex(wallet_c)
        if not all(x.lower().startswith("41") and len(x) == 42 for x in (a_hex, b_hex, c_hex)):
            raise ValueError("Invalid TRON address")
    except Exception as e:
        send_telegram(f"❌ Invalid TRON wallet address: <code>{str(e)[:300]}</code>")
        return

    if not MANUAL_WALLET_C_PRIVATE_KEY:
        log.error("MANUAL_WALLET_C_PRIVATE_KEY is not configured.")
        send_telegram(
            "❌ <b>MANUAL_WALLET_C_PRIVATE_KEY is missing.</b>\n\n"
            "Add it to GitHub Actions Secrets and expose it in the workflow environment."
        )
        return

    try:
        from tronpy.keys import PrivateKey
        c_key = PrivateKey(bytes.fromhex(MANUAL_WALLET_C_PRIVATE_KEY.strip().replace("0x", "")))
        derived_c = c_key.public_key.to_base58check_address()
    except Exception as e:
        log.exception("Could not load MANUAL_WALLET_C_PRIVATE_KEY.")
        send_telegram(f"❌ Wallet C private key is invalid: <code>{str(e)[:300]}</code>")
        return

    if derived_c != wallet_c:
        send_telegram(
            "❌ <b>Wallet C mismatch.</b>\n\n"
            f"The supplied Wallet C does not match the address derived from the configured "
            "<code>MANUAL_WALLET_C_PRIVATE_KEY</code>.\n\n"
            f"Configured key derives to: <code>{derived_c}</code>"
        )
        return
    ...
    cur = db.execute(
        "INSERT INTO execution_queue "
        "(wallet_a, wallet_b, wallet_c_address, wallet_c_private_key, prefix_match, suffix_match, status, created_at) "
        "VALUES (?, ?, ?, ?, 0, 0, 'approved', ?)",
        (wallet_a, wallet_b, wallet_c, MANUAL_WALLET_C_PRIVATE_KEY.strip().replace("0x", ""), now)
    )
    pair_id = cur.lastrowid
    db.execute(
        "INSERT OR REPLACE INTO monitor_state "
        "(pair_id, monitoring_started_at, first_trigger_tx_hash, first_trigger_timestamp, last_checked_at) "
        "VALUES (?, ?, NULL, NULL, ?)",
        (pair_id, now, now)
    )
    db.commit()
    ...
    send_telegram(
        f"✅ <b>Manual monitor activated — Pair #{pair_id}</b>\n\n"
        f"<b>Wallet A:</b> <code>{wallet_a}</code>\n"
        f"<b>Wallet B:</b> <code>{wallet_b}</code>\n"
        f"<b>Wallet C:</b> <code>{wallet_c}</code>\n\n"
        "👀 Monitoring now for <b>two confirmed A → B TRX transfers within 40 minutes</b>.\n"
        "The second transfer triggers Main → C → A automatically."
    )
    log.info("Manual monitor activated: pair #%s", pair_id)
