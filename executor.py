...
def execute_trigger_for_pair(row, first_tx, second_tx):
    ...
    try:
        ...
        tx1 = tron.trx.transfer(main_addr, row["wallet_c_address"], 1).build().sign(main_priv).broadcast().txid
        time.sleep(3)
        tx2 = tron.trx.transfer(row["wallet_c_address"], row["wallet_a"], 1).build().sign(c_priv).broadcast().txid
