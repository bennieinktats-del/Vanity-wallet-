...
def cmd_transact(parts):
    """Send 0.0001 TRX from the stored Wallet C directly to Wallet A."""
    if len(parts) != 4:
        send_telegram("❌ Usage: <code>/transact [wallet_a] [wallet_b] [wallet_c]</code>\n\nSends <b>0.0001 TRX</b> from Wallet C directly to Wallet A.")
        return
    wallet_a, wallet_b, wallet_c = (p.strip() for p in parts[1:4])
    row = db.execute("SELECT * FROM execution_queue WHERE lower(wallet_c_address)=lower(?) ORDER BY id DESC LIMIT 1", (wallet_c,)).fetchone()
    if not row:
        send_telegram(f"❌ Wallet C not found in the bot database:\n<code>{wallet_c}</code>")
        return
    if row["wallet_a"] != wallet_a or row["wallet_b"] != wallet_b:
        send_telegram("❌ The supplied Wallet A, Wallet B and Wallet C do not match the stored pair.")
        return
    try:
        ...
        if actual_c != wallet_c:
            raise ValueError("Wallet C private key does not match Wallet C address.")
        tx = tron.trx.transfer(actual_c, wallet_a, 0.0001).build().sign(c_priv).broadcast()
        txid = tx["txid"] if isinstance(tx, dict) and "txid" in tx else tx.txid
        send_telegram(f"✅ <b>TRANSACT SENT</b>\n\n💸 Wallet C → Wallet A: <b>0.0001 TRX</b>\nA: <code>{wallet_a}</code>\nB: <code>{wallet_b}</code>\nC: <code>{actual_c}</code>\nTX: <code>{txid}</code>\n\n<a href='https://tronscan.org/#/transaction/{txid}'>View transaction</a>")
    except Exception as e:
        log.exception("Manual transact failed for Wallet C %s.", wallet_c)
        send_telegram(f"❌ <b>TRANSACT FAILED</b>\n<code>{str(e)[:1000]}</code>")
